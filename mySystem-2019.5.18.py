    ...
    def add_into_tracker(self, mask, frame_color, cur_num):
        ...
        if cur_num == 1:  # 这个帧数非常关键，如果没有识别，去查thread里面的初始帧是0还是1
            ...
            if self.auto_detect_flag:
                # mask = mask[:, 0:280].copy()
                num, labels, stats, centroids = cv2.connectedComponentsWithStats(mask)
                # cv2.imshow('entrance', entrance)
                # cv2.waitKey(1)
                for i in range(num):
                    found = False
                    # if i > 0 and stats[i][4] > 80 and stats[i][0] > 5:  # 如果识别到面积大于200且不与任何一个roi沾边的对象
                    if i > 0 and stats[i][4] > 80 and self.in_roi(stats[i]):  # 如果识别到面积大于200且不与任何一个roi沾边的对象
...
class PlayerBox(QMainWindow, Ui_MainWindow):
    ...
    def open_filter(self):
        time.sleep(0.05)
        self.vth.CF.filter_flag = self.action_ContourFilter.isChecked()
