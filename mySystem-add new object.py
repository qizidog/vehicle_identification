...
class ColorThreshFilter:
    def __init__(self):
        ...
        self.theta = 0.774  # 默认的旋转角度
        self.leftCol, self.rightCol, self.upRow, self.downRow = 470, 1030, 580, 670  # 默认截取的窗口大小
    ...
    def add_into_tracker(self, mask, frame_color, cur_num):
        if cur_num == 0:  # 这个帧数非常关键，如果没有识别，很有可能是这里出了问题
            pass
            ...
            #         self.colors.append((np.random.randint(64, 255), np.random.randint(64, 255), np.random.randint(64, 255)))
            ...
            for i, newbox in enumerate(boxes):
                ...
                cv2.rectangle(frame_color, p1, p2, self.colors[i], 2, 1)
            entrance = mask[:, 0:150].copy()
            num, labels, stats, centroids = cv2.connectedComponentsWithStats(entrance)
            cv2.imshow('entrance', entrance)
            for i in range(num):
                found = False
                if i > 0 and stats[i][4] > 400 and stats[i][0] > 5:  # 如果识别到面积大于400且不沾边的对象
                    for j in boxes:
                        if j[0] < 150:
                            cent1 = [(stats[i][0]+stats[i][2])/2, (stats[i][1]+stats[i][3])/2]
                            cent2 = [(j[0]+j[2])/2, (j[1]+j[3])/2]
                            area1 = stats[i][2] * stats[i][3]
                            area2 = j[2] * j[3]
                            if self.isSameCar(cent1, cent2, area1, area2):
                                found = True
                                break
                    if not found:
                        bbox = stats[i][:4]
    ...
    def isSameCar(self, cent1, cent2, area1, area2):
        cond1 = np.linalg.norm(np.array(cent1) - np.array(cent2)) < 30  # 中心点距离小于30个像素
        # cond2 = min(area1, area2)/max(area1, area2) > 0.7  # 面积相差不大与30%
    ...
    def main_proceed(self, frame, cur_num):
        ...
        frame_adjusted = frame.copy()  # 没有进行过任何筛选的图，备份一个，用来返回后传入ColorThresholder里面查看阈值设置的效果
        ...
        frame_color = self.add_into_tracker(mask, frame_color, cur_num)
...
class PlayerBox(QMainWindow, Ui_MainWindow):
        ...
        while 1:
            ...
            if k == ord('c'):  # 重置图片
                ...
                try:
                    ...
                    self.CTF.theta = theta
                    print(theta)  # 赋值
                    cv2.destroyWindow('rotate frame')
                    self.show_frame(frame2, 0, 0)
                    break
            elif k == 27:  # 退出不做修改
                cv2.destroyWindow('rotate frame')
                break
    ...
    def select_view(self):
        ...
        frame2 = frame.copy()
        rows, cols, ch = frame.shape
        M = cv2.getRotationMatrix2D((cols/2, rows/2), self.CTF.theta, 1)  # 三个参数分别是旋转中心，旋转角度，比例
        frame = cv2.warpAffine(frame, M, (cols, rows))

        frame = cv2.resize(frame, None, fx=0.8, fy=0.8, interpolation=cv2.INTER_CUBIC)
        bbox = list(cv2.selectROI('selectROI', frame))
        cv2.destroyWindow('selectROI')
        bbox[0] = int(1.25 * bbox[0])
        bbox[1] = int(1.25 * bbox[1])
        bbox[2] = int(1.25 * bbox[2])
        bbox[3] = int(1.25 * bbox[3])
        self.CTF.leftCol, self.CTF.rightCol, self.CTF.upRow, self.CTF.downRow = bbox[0], bbox[0]+bbox[2], bbox[1], bbox[1]+bbox[3]
        print(bbox)
        self.show_frame(frame2, 0, 0)
